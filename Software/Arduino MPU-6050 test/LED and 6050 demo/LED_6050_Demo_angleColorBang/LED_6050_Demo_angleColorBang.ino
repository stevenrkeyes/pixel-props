...
void loop()
{
  ...
  long brightness = ((long) accel_t_gyro.value.y_accel) + 17000;
  brightness = 30 * brightness;
  brightness = brightness / 34000;
  //Serial.print(brightness, DEC);
  //Serial.print("\t");
  //Serial.println(accel_t_gyro.value.y_accel, DEC);

  int gyroConversionFactor = 131; // LSB/(degree/s)
  int yangVel = accel_t_gyro.value.y_gyro / gyroConversionFactor;
  int zangVel = accel_t_gyro.value.z_gyro / gyroConversionFactor;
  // the angular speed of the poi/staff around the spinner's hand
  // note that we lose direction information when we perform this calc
  int angular_speed = sqrt(yangVel*yangVel + zangVel*zangVel);
  
  Serial.println(angular_speed, DEC);
  
  if (accel_t_gyro.value.y_accel < -16000 || accel_t_gyro.value.y_accel > 16000)
  {
    for(int i=0; i<strip.numPixels(); i++)
    {
      strip.setPixelColor(i, strip.Color((int) 17,17,17));
    }
  }
  else
  {
    for(int i=0; i<strip.numPixels(); i++)
    {
      strip.setPixelColor(i, strip.Color((int) brightness,3,3));
    }
  }
  strip.show();
